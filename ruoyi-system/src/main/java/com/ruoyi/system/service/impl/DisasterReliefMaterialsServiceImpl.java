...
@Service
public class DisasterReliefMaterialsServiceImpl extends
        ServiceImpl<DisasterReliefMaterialsMapper, DisasterReliefMaterials>
        implements DisasterReliefMaterialsService, DataExportStrategy{
    ...
    public List<DisasterReliefMaterials> importExcelDisasterReliefMaterials(MultipartFile file, String userName, String eqId) throws IOException {
        ...
        DisasterReliefMaterialsListener listener = new DisasterReliefMaterialsListener(baseMapper, totalRows, userName);
        // 读取Excel文件，从第4行开始
        EasyExcel.read(inputStream, DisasterReliefMaterials.class, listener).headRowNumber(Integer.valueOf(2)).sheet().doRead();
        // 获取解析后的数据
        List<DisasterReliefMaterials> list = listener.getList();
        // 将解析后的数据保存到数据库
        // 遍历解析后的数据，根据地震时间与地震名称查找eqList表中的earthquakeId
        for (DisasterReliefMaterials data : list) {
...
//            data.setMagnitude(earthquakeIdByTimeAndPosition.get(0).getMagnitude());
    ...
    public IPage getPage(RequestBTO requestBTO) {
        ...
        LambdaQueryWrapper<DisasterReliefMaterials> queryWrapper =
                Wrappers.lambdaQuery(DisasterReliefMaterials.class).like(DisasterReliefMaterials::getEarthquakeId, requestParam);
        return page(DisasterReliefMaterialsPage, queryWrapper);
    }

    @Override
    public List<?> exportExcelGetData(RequestBTO requestBTO) {
        String [] ids = requestBTO.getIds();
        List<DisasterReliefMaterials> list;
        if (ids == null || ids.length == 0) {
            list = this.list().stream()
                    .sorted(Comparator.comparing(DisasterReliefMaterials::getSystemInsertTime, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    public IPage<DisasterReliefMaterials> searchData(RequestBTO requestBTO) {
        ...
        if (MessageConstants.CONDITION_SEARCH.equals(requestBTO.getCondition())) {
            ...
            queryWrapper.eq(DisasterReliefMaterials::getEarthquakeId, eqId)
        ...
        if (requestBTO.getCondition().equals(MessageConstants.CONDITION_FILTER)) {
            ...
            if (requestBTO.getFormVO().getEarthquakeAreaName() != null && !requestBTO.getFormVO().getEarthquakeAreaName().isEmpty()) {
                queryWrapper.like(DisasterReliefMaterials::getEarthquakeAreaName, requestBTO.getFormVO().getEarthquakeAreaName())
            ...
            if (requestBTO.getFormVO().getOccurrenceTime() != null && !requestBTO.getFormVO().getOccurrenceTime().isEmpty()) {
                ...
                queryWrapper.between(DisasterReliefMaterials::getSubmissionDeadline, startDate, endDate)
                        .eq(DisasterReliefMaterials::getEarthquakeId, eqId);
            }
        }
