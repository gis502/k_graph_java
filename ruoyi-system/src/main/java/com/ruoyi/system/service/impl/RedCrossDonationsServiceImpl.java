...
@Service
public class RedCrossDonationsServiceImpl extends
        ServiceImpl<RedCrossDonationsMapper, RedCrossDonations>
...
// 重新获取 InputStream
        ...
        RedCrossDonationsListener listener = new RedCrossDonationsListener(baseMapper, actualRows, userName);
        // 读取Excel文件，从第4行开始
        EasyExcel.read(inputStream, RedCrossDonations.class, listener).headRowNumber(Integer.valueOf(2)).sheet().doRead();
        // 获取解析后的数据
        List<RedCrossDonations> list = listener.getList();
        // 将解析后的数据保存到数据库
        // 遍历解析后的数据，根据地震时间与地震名称查找eqList表中的earthquakeId
        for (RedCrossDonations data : list) {
...
//            data.setMagnitude(earthquakeIdByTimeAndPosition.get(0).getMagnitude());
    ...
    public IPage getPage(RequestBTO requestBTO) {
        ...
        LambdaQueryWrapper<RedCrossDonations> queryWrapper =
                Wrappers.lambdaQuery(RedCrossDonations.class)
                        .like(RedCrossDonations::getEarthquakeId, requestParam);
        return this.page(redCrossDonationsPage, queryWrapper);
    }

    @Override
    public List<?> exportExcelGetData(RequestBTO requestBTO) {
        String[] ids = requestBTO.getIds();
        List<RedCrossDonations> list;
        if (ids == null || ids.length == 0) {
            list = this.list().stream()
                    .sorted(Comparator.comparing(RedCrossDonations::getRecordTime, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    public IPage<RedCrossDonations> searchData(RequestBTO requestBTO) {
        ...
        if (MessageConstants.CONDITION_SEARCH.equals(requestBTO.getCondition())) {
            ...
            queryWrapper.eq(RedCrossDonations::getEarthquakeId, eqId)
        ...
        if (requestBTO.getCondition().equals(MessageConstants.CONDITION_FILTER)) {
            ...
            if (requestBTO.getFormVO().getEarthquakeAreaName() != null && !requestBTO.getFormVO().getEarthquakeAreaName().isEmpty()) {
                queryWrapper.like(RedCrossDonations::getEarthquakeAreaName, requestBTO.getFormVO().getEarthquakeAreaName())
            ...
            if (requestBTO.getFormVO().getOccurrenceTime() != null && !requestBTO.getFormVO().getOccurrenceTime().isEmpty()) {
                ...
                queryWrapper.between(RedCrossDonations::getSubmissionDeadline, startDate, endDate)
                        .eq(RedCrossDonations::getEarthquakeId, eqId);
            }
        }
