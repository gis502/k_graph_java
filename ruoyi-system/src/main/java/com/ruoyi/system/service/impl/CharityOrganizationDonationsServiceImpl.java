...
@Service
public class CharityOrganizationDonationsServiceImpl extends
        ServiceImpl<CharityOrganizationDonationsMapper, CharityOrganizationDonations>
...
// 重新获取 InputStream
        ...
        CharityOrganizationDonationsListener listener = new CharityOrganizationDonationsListener(baseMapper, actualRows, userName);
        // 读取Excel文件，从第4行开始
        EasyExcel.read(inputStream, CharityOrganizationDonations.class, listener).headRowNumber(Integer.valueOf(2)).sheet().doRead();
        // 获取解析后的数据
        List<CharityOrganizationDonations> list = listener.getList();
        // 将解析后的数据保存到数据库
...
//            data.setMagnitude(earthquakeIdByTimeAndPosition.get(0).getMagnitude());
    ...
    public IPage getPage(RequestBTO requestBTO) {
        ...
        LambdaQueryWrapper<CharityOrganizationDonations> queryWrapper =
                Wrappers.lambdaQuery(CharityOrganizationDonations.class)
                        .like(CharityOrganizationDonations::getEarthquakeId, requestParam);
        return this.page(charityOrganizationDonationsPage, queryWrapper);
    }
    ...
    public List<?> exportExcelGetData(RequestBTO requestBTO) {
        ...
        if (ids == null || ids.length == 0) {
            list = this.list().stream()
                    .sorted(Comparator.comparing(CharityOrganizationDonations::getRecordTime, Comparator.nullsLast(Comparator.naturalOrder()))
