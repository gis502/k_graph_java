...
@Service
public class LargeSpecialRescueEquipmentServiceImpl extends
        ServiceImpl<LargeSpecialRescueEquipmentMapper, LargeSpecialRescueEquipment>
    ...
    public List<LargeSpecialRescueEquipment> importExcelLargeSpecialRescueEquipment(MultipartFile file, String userName, String eqId) throws IOException {
        ...
        for (LargeSpecialRescueEquipment data : list) {
...
//            data.setMagnitude(earthquakeIdByTimeAndPosition.get(0).getMagnitude());
    ...
    public IPage getPage(RequestBTO requestBTO) {
        ...
        LambdaQueryWrapper<LargeSpecialRescueEquipment> queryWrapper =
                Wrappers.lambdaQuery(LargeSpecialRescueEquipment.class).like(LargeSpecialRescueEquipment::getEarthquakeId, requestParam);
        return page(LargeSpecialRescueEquipmentPage, queryWrapper);
    }
    ...
    public List<?> exportExcelGetData(RequestBTO requestBTO) {
        ...
        if (ids == null || ids.length == 0) {
            list = this.list().stream()
                    .sorted(Comparator.comparing(LargeSpecialRescueEquipment::getSystemInsertTime, Comparator.nullsLast(Comparator.naturalOrder()))
    ...
    public IPage<LargeSpecialRescueEquipment> searchData(RequestBTO requestBTO) {
        ...
        LambdaQueryWrapper<LargeSpecialRescueEquipment> queryWrapper = Wrappers.lambdaQuery(LargeSpecialRescueEquipment.class)
                ...
                .or().like(LargeSpecialRescueEquipment::getEarthquakeTime, requestParams) // 地震时间
                .or().like(LargeSpecialRescueEquipment::getEarthquakeAreaName, requestParams) // 震区（县/区）
                .or().like(LargeSpecialRescueEquipment::getSubmissionDeadline, requestParams) // 统计截止时间
